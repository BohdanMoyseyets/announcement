...
const Items = (props) => {
    var itemsElements = props.itemsData.map((item, index) => <Item showSimilarItem={props.showSimilarItem} showDetailsItem={props.showDetailsItem} deleteItem={props.deleteItem} editItem={props.editItem} editItemChangeTitle={props.editItemChangeTitle} editItemChangeDesc={props.editItemChangeDesc} editTitleText={props.editTitleText} editDescText={props.editDescText} itemsData={props.itemsData} data={item} index={index} />);
    let newItemTitle = React.createRef();
    let newItemDesc = React.createRef();
    let searchItemTitle = React.createRef();
    var itemsElements2 = props.itemsData.map((item, index) => { if (item.title.includes(props.searchItemTitle)) { return (<Item showSimilarItem={props.showSimilarItem} showDetailsItem={props.showDetailsItem} deleteItem={props.deleteItem} editItem={props.editItem} editItemChangeTitle={props.editItemChangeTitle} editItemChangeDesc={props.editItemChangeDesc} editTitleText={props.editTitleText} editDescText={props.editDescText} itemsData={props.itemsData} data={item} index={index} />) } else return null });;
    console.log(itemsElements2);
        ...
        <div className={style.items}>
            <div className={style.add_item}>
                <div className={style.add_item_title}>
                    <label>Title</label>
                    <input onChange={onChangeTitle} type="text" ref={newItemTitle} value={props.newTitleText} />
                </div>
                <div className={style.add_item_desc}>
                    <label>Description</label>
                    <textarea onChange={onChangeDesc} ref={newItemDesc} value={props.newDescText}></textarea>
                </div>
                <button className={style.add_item_button} onClick={onAddItem}>add new announcement</button>
            </div>
            <h1>Announcements</h1>
            <div>
